...
We will use two main functions for estimating such models. One is the function `tseries::garch()` which may be used for relatively simple fittings and the other are sets of functions from the package `fGarch` which offers more options including a diverse set of distributions for the innovations.
